...
vi.mock('../components/settings/BillInboxConfig', () => ({ BillInboxConfig: () => <div>邮件自动收账</div> }))
...
it('places automatic account sync above manual WebDAV backup', () => {
  ...
  const inbox = screen.getByText('邮件自动收账')
  expect(account.compareDocumentPosition(webdav) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  expect(account.compareDocumentPosition(inbox) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
})
